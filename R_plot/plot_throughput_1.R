...
library(ggplot2)
single_link_throughput_1_2_0<-read.table("output/out1_0")
time_1<-single_link_throughput_1_2_0$V1
data_1<-single_link_throughput_1_2_0$V2
g1<-ggplot(data=NULL,aes(x=time_1,y=data_1))
graph_1_2_0<-g1+geom_point(color="darkred",alpha=0.1)+labs(title="1-0",x="time(s)",y="throughput(kb/s)")

single_link_throughput_0_2_1<-read.table("output/out0_1")
time_2<-single_link_throughput_0_2_1$V1
data_2<-single_link_throughput_0_2_1$V2
g2<-ggplot(data=NULL,aes(x=time_2,y=data_2))
graph_0_2_1<-g2+geom_point(color="darkblue",alpha=0.1,aes(color="blue"))+labs(title="0-1",x="time(s)",y="throughput(kb/s)")
...
graph_all<-g3+geom_point(aes(color=direction),alpha=0.1)+labs(title="average throughput",x="time(s)",y="throughput(kb/s)")